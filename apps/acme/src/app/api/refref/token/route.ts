...
import { getRefRefConfig, getRefRefConfigSync } from '@/lib/refref-runtime-config';
  ...
  try {
    ...
    try {
      ...
      if (configCookie && secretCookie) {
        ...
        console.log('RefRef config from cookies:', { productId: config.productId, programId: config.programId });
      } else {
        // Fall back to regular config if no cookies
        config = await getRefRefConfig();
        console.log('RefRef config from getRefRefConfig:', { productId: config.productId, programId: config.programId });
      }
    } catch (e) {
      // If cookie reading fails, use regular config
      config = await getRefRefConfig();
      console.log('RefRef config from fallback:', { productId: config.productId, programId: config.programId });
    }
